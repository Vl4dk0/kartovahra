(function(){
  ...
  const dealerIndex = () => state.history.length % Math.max(state.players.length,1);
  ...
  function render(){
    if(state.over){ renderGame(); showView("game"); openOverlay(); }
    else if(state.started){ closeOverlay(); renderGame(); showView("game"); }
    else { closeOverlay(); renderSetup(); showView("setup"); }
  }
  ...
  function renderGame(){
    ...
    const dsuit = suitOf(di), dred = isRed(di);
    const dcolor = dred ? "var(--red)" : "var(--ink)";
    const idx = initial(d.name) + dsuit;
    $("#dcIdxTL").textContent = idx; $("#dcIdxBR").textContent = idx;
    $("#dcPip").textContent = dsuit;
    $("#dcIdxTL").style.color = $("#dcIdxBR").style.color = $("#dcPip").style.color = dcolor;
    ...
    if(state.history.length===0){
      ...
      state.history.forEach((round,r)=>{
        ...
        for(let i=0;i<n;i++){
          ...
          let cls = "delta", txt;
          const raw = round[i]||0;
          if(after===100){ after=90; cls+=" drop"; }
          else if(after>100){ cls+=" bust"; }
          else if(raw===0){ cls+=" zero"; }
          run[i]=after;
          txt = (raw>0?"+":"") + raw + (after===90 && before+raw===100 ? " ↓" : "");
  ...
  function openRound(){
    ...
    const di = dealerIndex();
    $("#sheetTitle").textContent = "Kolo " + (state.history.length+1) + " — body";
    $("#sheetDeal").innerHTML = `Mieša <b>${escapeHtml(state.players[di].name)}</b>. Zapíš, koľko kto nazbieral.`;
    const t = totals();
    $("#sheetRows").innerHTML = state.players.map((p,i)=>`
      <div class="prow">
        <div class="prow__id">
    ...
    $("#sheetRows").querySelectorAll(".pts").forEach(inp=>{
      inp.addEventListener("focus", ()=> inp.select());
    });
  }
  function closeRound(){ $("#sheetBack").classList.remove("is-open"); }
  ...
  function saveRound(){
    ...
    state.players.forEach((p,i)=>{
      ...
      if(reached === 100){ toast(`<b>${escapeHtml(p.name)}</b> má presne 100 → späť na 90!`); }
      else if(reached > 100){ busted.push(i); }
    });
    ...
    if(busted.length){
      ...
      const c = $("#dealerCard"); c.classList.remove("card--deal"); void c.offsetWidth; c.classList.add("card--deal");
    }
  }
  ...
  function openOverlay(){
    ...
    const order = state.players.map((p,i)=>({i, t:t[i]})).sort((a,b)=>a.t-b.t);
    const minT = order[0].t;
    $("#standings").innerHTML = order.map((o,rank)=>{
    ...
    requestAnimationFrame(()=>{ ov.classList.add("shake"); setTimeout(()=>ov.classList.remove("shake"),600); });
  }
  ...
  function addPlayer(){
    ...
    while(state.players.some(p=>p.name===name)){ name = base+" "+(k++); }
  ...
  document.addEventListener("click", e=>{
    ...
    switch(a){
      ...
      case "remove-player": state.players.splice(+el.dataset.i,1); save(); renderSetup(); break;
      case "start":
        if(state.players.length>=2){ state.started=true; state.history=[]; state.over=false; save(); render();
          const c=$("#dealerCard"); c.classList.remove("card--deal"); void c.offsetWidth; c.classList.add("card--deal"); }
        break;
      case "open-round": openRound(); break;
      case "cancel-round": closeRound(); break;
      case "save-round": saveRound(); break;
      case "step": {
        const inp = $("#pts-"+el.dataset.i);
        let v = parseInt(inp.value,10); if(isNaN(v)) v=0;
        v += (+el.dataset.d); inp.value = v;
        break;
      }
      case "undo":
        if(state.history.length){ state.history.pop(); state.over=false; save(); render(); }
        break;
      case "undo-from-over":
        if(state.history.length){ state.history.pop(); state.over=false; save(); render(); }
        break;
      case "end-game":
        if(confirm("Ukončiť hru a vrátiť sa k hráčom? Skóre sa zmaže.")){
          state.started=false; state.over=false; state.history=[]; save(); render();
        }
        break;
      case "rematch":
        state.history=[]; state.over=false; state.started=true; save(); render();
        { const c=$("#dealerCard"); c.classList.remove("card--deal"); void c.offsetWidth; c.classList.add("card--deal"); }
        break;
      case "new-players":
        state.history=[]; state.over=false; state.started=false; save(); render();
        break;
    }
  });
  ...
  $("#sheetBack").addEventListener("click", e=>{ if(e.target===$("#sheetBack")) closeRound(); });
  document.addEventListener("keydown", e=>{ if(e.key==="Escape") closeRound(); });
